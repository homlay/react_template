...
function AudioUploader() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/process_meeting', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 space-y-6">
      <div className="flex flex-col items-center space-y-6">
        ...
        {/* 错误提示 */}
        {error && (
          <div className="w-full p-4 text-red-700 bg-red-50 rounded-lg border-l-4 border-red-500 shadow-sm">
            {error}
          </div>
        )}
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.tasks?.map((task, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-blue-200 hover:bg-blue-50 transition-colors duration-200">
                      <div className="font-medium">{task.description}</div>
                      <div className="flex justify-between mt-2 text-xs text-gray-500">
                        <span>负责人: <span className="font-medium">{task.assigned_to}</span></span>
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.decisions?.map((decision, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-green-200 hover:bg-green-50 transition-colors duration-200">
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.questions?.map((q, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-purple-200 hover:bg-purple-50 transition-colors duration-200">
                      <div className="font-medium">{q.question}</div>
                      <div className="flex justify-between mt-2 text-xs text-gray-500">
                        <span>提问者: <span className="font-medium">{q.raised_by}</span></span>
                        <span className={`px-2 py-1 rounded-full ${
                          q.status === "未回答" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"
                        }`}>
                          {q.status}
                        </span>
                      </div>
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.insights?.map((insight, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-amber-200 hover:bg-amber-50 transition-colors duration-200">
                      {insight.description}
                    </li>
                  ))}
                </ul>
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.deadlines?.map((deadline, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-red-200 hover:bg-red-50 transition-colors duration-200 flex justify-between">
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.Breakdown.attendees?.map((person, idx) => (
                    <span key={idx} className="text-sm px-4 py-2 bg-gray-50 border border-gray-100 rounded-full hover:bg-indigo-50 hover:border-indigo-200 transition-colors duration-200">
                      {person.name}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.follow_ups?.map((item, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border border-gray-100 hover:border-cyan-200 hover:bg-cyan-50 transition-colors duration-200">
                      <div>{item.description}</div>
                      <div className="flex justify-between mt-2 text-xs text-gray-500">
                        <span>负责人: <span className="font-medium">{item.owner}</span></span>
              ...
              <div className="bg白 p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ul className="space-y-3">
                  {result.Breakdown.risks?.map((risk, idx) => (
                    <li key={idx} className="text-sm bg-gray-50 p-3 rounded-md border-l-4 border-l-orange-400 border border-gray-100 hover:bg-orange-50 transition-colors duration-200">
                      {risk.description}
                    </li>
                  ))}
                </ul>
              ...
              <div className="bg-white p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <ol className="list-decimal list-inside space-y-2">
                  {result.Breakdown.agenda?.map((item, idx) => (
                    <li key={idx} className="text-sm p-3 bg-gray-50 rounded-md hover:bg-emerald-50 transition-colors duration-200">{item}</li>
                  ))}
                </ol>
              </div>
            </div>
